<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=660000,b30000&height=200&section=header&text=Gokul%20Krishna%20M&fontSize=32&fontColor=ffffff&animation=fadeIn&fontAlignY=65&desc=Fullstack%20Java%20%26%20MERN%20Stack%20Developer&descAlignY=80&descSize=16" />

<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&duration=3000&pause=1000&color=B30000&center=true&vCenter=true&width=650&lines=Java+17+%7C+Spring+Boot+%7C+MERN+Stack;React+18+%7C+Node.js+%7C+Express+%7C+MongoDB;AWS+(EC2%2C+CloudFront)+%7C+Docker+%7C+CI%2FCD;Building+GenAI-powered+Full+Stack+Apps" alt="Typing SVG" />
</a>

<br/>

![B.Tech CSE](https://img.shields.io/badge/B.Tech-CSE%20%40%20Dr.%20MGR%20University-B30000?style=flat-square)
![Graduate](https://img.shields.io/badge/Graduated-2024-B30000?style=flat-square)
![Status](https://img.shields.io/badge/Status-Open%20to%20Work-B30000?style=flat-square)
![Location](https://img.shields.io/badge/Location-Chennai%2C%20India-B30000?style=flat-square)

<br/>

[![Portfolio](https://img.shields.io/badge/Portfolio-B30000?style=for-the-badge&logo=vercel&logoColor=white)](https://gokulkrishna-portfolio.netlify.app/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-B30000?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/gokulkrishna08)
[![GitHub](https://img.shields.io/badge/GitHub-B30000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/gokulkrishna12)

<br/>

![Profile Views](https://komarev.com/ghpvc/?username=gokulkrishna12&color=B30000&style=flat-square&label=Profile+Views)
![Followers](https://img.shields.io/github/followers/gokulkrishna12?color=B30000&style=flat-square&label=Followers)
![Stars](https://img.shields.io/github/stars/gokulkrishna12?color=B30000&style=flat-square&label=Stars)

</div>

---

### 🧭 About Me

I'm a **Java & MERN Full Stack Developer** and a **2024 B.Tech CSE graduate from Dr. M.G.R. Educational and Research Institute, Chennai**, open to full-time software engineering roles.

I design, build, and deploy high-performance applications spanning **Java 17 / Spring Boot** and the **MERN stack (MongoDB, Express.js, React 18, Node.js)**. My background includes architecting secure RESTful APIs, building stateless JWT authentication flows, integrating payment workflows (Razorpay with cryptographic signature verification), and delivering responsive, mobile-first interfaces.

I handle end-to-end deployments across **AWS (EC2, CloudFront, RDS, S3)** with **Nginx reverse proxies**, **Docker containerization**, and automated **GitHub Actions CI/CD pipelines**, while integrating **Google Gemini GenAI** into production workflows.

**🎯 Open to:** Full Stack Developer (Java / MERN) · Backend Developer (Spring Boot / Node.js) · Frontend Developer (React.js)

---

### 🛠️ Tech Stack

**💻 Languages**

![Java](https://img.shields.io/badge/Java%2017-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![SCSS](https://img.shields.io/badge/SCSS-CC6699?style=for-the-badge&logo=sass&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)

**🎨 Frontend**

![React](https://img.shields.io/badge/React%2018-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![React Router](https://img.shields.io/badge/React%20Router-CA4245?style=for-the-badge&logo=reactrouter&logoColor=white)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white)
![Bootstrap](https://img.shields.io/badge/Bootstrap-7952B3?style=for-the-badge&logo=bootstrap&logoColor=white)
![Axios](https://img.shields.io/badge/Axios-5A29E4?style=for-the-badge&logo=axios&logoColor=white)

**⚙️ Backend & APIs**

![Spring Boot](https://img.shields.io/badge/Spring%20Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white)
![Spring Security](https://img.shields.io/badge/Spring%20Security-6DB33F?style=for-the-badge&logo=springsecurity&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Express](https://img.shields.io/badge/Express.js-000000?style=for-the-badge&logo=express&logoColor=white)
![JWT](https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=jsonwebtokens&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)

**🗄️ Databases & ORM/ODM**

![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white)
![Mongoose](https://img.shields.io/badge/Mongoose-880000?style=for-the-badge&logo=mongoose&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL%208.0-4479A1?style=for-the-badge&logo=mysql&logoColor=white)

**☁️ Cloud, DevOps & Infrastructure**

![AWS](https://img.shields.io/badge/AWS%20(EC2%2C%20CloudFront)-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Nginx](https://img.shields.io/badge/Nginx-009639?style=for-the-badge&logo=nginx&logoColor=white)
![Linux](https://img.shields.io/badge/Ubuntu%20Linux-E95420?style=for-the-badge&logo=ubuntu&logoColor=white)
![Vercel](https://img.shields.io/badge/Vercel-000000?style=for-the-badge&logo=vercel&logoColor=white)
![Render](https://img.shields.io/badge/Render-46E3B7?style=for-the-badge&logo=render&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub%20Actions-2088FF?style=for-the-badge&logo=githubactions&logoColor=white)

**🧰 Tools & Integrations**

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)
![Cloudinary](https://img.shields.io/badge/Cloudinary-3448C5?style=for-the-badge&logo=cloudinary&logoColor=white)
![Razorpay](https://img.shields.io/badge/Razorpay-02042B?style=for-the-badge&logo=razorpay&logoColor=white)

---

### 🤖 AI / ML & GenAI Integration

| Domain | Proficiency | Details |
|---|---|---|
| LLM API Integration | Applied | Integrated Google Gemini API via async REST endpoints for real-time, context-aware product suggestions |
| Prompt-driven Features | Applied | Built AI-assisted shopping suggestions inside a full production e-commerce flow |
| JSON Streaming | Applied | Used Jackson JSON streaming to handle real-time LLM responses |

---

### 🚀 Featured Projects

<details>
<summary><b>🛒 GK ShopEase — AI-Powered Full-Stack E-Commerce Platform (Java / Spring Boot)</b></summary>
<br/>

End-to-end cloud e-commerce platform with a decoupled React frontend and modular Spring Boot backend, deployed across AWS infrastructure with an integrated GenAI shopping assistant.

| | |
|---|---|
| **Stack** | Java 17, Spring Boot 3, React 18, Vite, MySQL (AWS RDS), Docker |
| **Scale** | 10+ documented RESTful endpoints (Swagger/OpenAPI 3) |
| **Security** | JWT auth + Role-Based Access Control via Spring Security |
| **AI** | Google Gemini LLM integration for contextual product suggestions |
| **Deployment** | AWS EC2 (backend), S3 + CloudFront + SSL (frontend), GitHub Actions CI/CD |
| **Repository** | [Frontend](https://github.com/gokulkrishna12/E-Commerce-FRONTEND) · [Backend](https://github.com/gokulkrishna12/E-Commerce-BACKEND) |

Automated dual CI/CD pipelines handle Maven builds, SCP transfers, systemd service management, and CDN cache invalidation.

</details>

<details>
<summary><b>🏋️‍♂️ GK's Fitness Shop — Production MERN E-Commerce Platform</b></summary>
<br/>

A responsive, mobile-first e-commerce platform tailored for fitness enthusiasts, featuring Razorpay payment processing, real-time inventory management, and a full AWS deployment pipeline.

| | |
|---|---|
| **Stack** | React 18, Node.js, Express.js, MongoDB Atlas, Razorpay API, Nginx, Ubuntu Linux |
| **Security** | Cryptographic signature verification for payment checkout + custom OTP-based user authentication |
| **Data Integrity** | Real-time inventory synchronization in MongoDB during concurrent payments and order cancellations |
| **Deployment** | AWS EC2 backend with Nginx reverse proxy, frontend globally distributed via AWS CloudFront CDN |
| **Repository** | [Frontend](https://github.com/gokulkrishna12/Gk-s-Fitness-Shop-FRONTEND) · [Backend](https://github.com/gokulkrishna12/Gk-s-Fitness-Shop-BACKEND) |

</details>

<details>
<summary><b>💰 Expense Tracker — Secure Full Stack Finance App</b></summary>
<br/>

A full stack expense management app with JWT authentication, category-wise tracking, and dashboard analytics.

| | |
|---|---|
| **Stack** | React, Vite, Bootstrap, Chart.js · Spring Boot 3, Spring Security, JPA, MySQL (Aiven Cloud) |
| **Security** | Spring Security + JWT-based authentication |
| **Features** | CRUD expenses, search/filter, CSV export, pie-chart analytics, dark/light theme |
| **Deployment** | Vercel (frontend), Railway (backend) |
| **Repository** | [Frontend](https://github.com/gokulkrishna12/Expense-Tracker-Frontend) · [Backend](https://github.com/gokulkrishna12/Expense-Tracker-Backend) |

</details>

<details>
<summary><b>🏋️ Fit-Track Pro Gym — Full Stack Workout Tracker</b></summary>
<br/>

A dynamic workout tracking app with an interactive React dashboard, custom glassmorphism styling, and secure stateless session handling.

| | |
|---|---|
| **Stack** | React 18, Vite, custom SCSS · Node.js, Express.js |
| **Security** | Stateless JWT-based authentication and authorization |
| **Features** | Dynamic dashboard for routine logging, RESTful CRUD APIs, fully responsive mobile-first UI |
| **Deployment** | Vercel (frontend), Render (backend) |
| **Repository** | [Frontend](https://github.com/gokulkrishna12/Fit-Track-FRONTEND) · [Backend](https://github.com/gokulkrishna12/Fit-Track-BACKEND) |

</details>

<details>
<summary><b>📦 Inventory Management System</b></summary>
<br/>

An end-to-end inventory control web application with a decoupled architecture and real-time MongoDB item updates.

| | |
|---|---|
| **Stack** | React 18 (Vite), React Router, Axios · Node.js, Express.js · MongoDB Atlas |
| **Features** | Comprehensive stock control, decoupled client-server architecture, real-time CRUD operations |
| **Deployment** | Vercel (frontend), Render (backend) |
| **Repository** | [Frontend](https://github.com/gokulkrishna12/Inventory-Management-FRONTEND) · [Backend](https://github.com/gokulkrishna12/Inventory-Management-BACKEND) |

</details>

<details>
<summary><b>👥 Employee Management System</b></summary>
<br/>

A classic full stack CRUD application for managing employee records.

| | |
|---|---|
| **Stack** | React.js · Spring Boot · MySQL |
| **Features** | Add/view/update/delete employees, RESTful API integration, responsive UI |
| **Repository** | [Frontend](https://github.com/gokulkrishna12/Employee-Management-system-FRONTEND) · [Backend](https://github.com/gokulkrishna12/Employee-Management-system-BACKEND) |

</details>

---

### 🎯 Current Focus

```yaml
learning:
  - Advanced microservices with Spring Boot & distributed Node.js architecture
building:
  - Enterprise-grade Java & MERN applications with automated CI/CD pipelines
exploring:
  - Generative AI integrations, real-time streaming, and high-availability cloud setups
open_to:
  - Full Stack Developer (Java / MERN) / Software Engineer roles
```

---

### Activities

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://cdn.jsdelivr.net/gh/gokulkrishna12/gokulkrishna12@output/github-contribution-grid-snake-dark.svg?v=10">
    <source media="(prefers-color-scheme: light)" srcset="https://cdn.jsdelivr.net/gh/gokulkrishna12/gokulkrishna12@output/github-contribution-grid-snake.svg?v=10">
    <img alt="GitHub Contribution Snake" src="https://cdn.jsdelivr.net/gh/gokulkrishna12/gokulkrishna12@output/github-contribution-grid-snake.svg?v=10">
  </picture>
</div>

---

### 📫 Connect With Me

<div align="center">

[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:gokuldinesh32@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/gokulkrishna08)
[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white)](https://gokulkrishna-portfolio.netlify.app/)

</div>

---

<div align="center">

*"Code is easy. Shipping something people can actually use — that's the real work."*

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=660000,b30000&height=200&section=footer&text=Gokul%20Krishna%20M&fontSize=32&fontColor=ffffff&animation=fadeIn&fontAlignY=65&desc=Fullstack%20Java%20%26%20MERN%20Stack%20Developer&descAlignY=80&descSize=16" />

</div>
